...
export const useAppStore = defineStore('app', () => {
  ...
  const isAnalysisRunning = computed(() =>
    !!selectedSourceTask.value && selectedSourceTask.value.status === 'running'
  )
  ...
  const startFullAnalysisWorkflow = async (sourceId: string, modelId: string, analysisType: string = 'object_detection') => {
    try {
      globalLoading.value = true
