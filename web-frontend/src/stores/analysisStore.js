...
export const useAnalysisStore = defineStore('analysis', () => {
    ...
    const analysisTypes = ref([
        ...
        { id: 'instance_segmentation', name: 'Instance Segmentation', enabled: false }
    ]);
    // computed
    const runningTasks = computed(() => analysisTasks.value.filter(task => task.status === 'running'));
    const loadedModels = computed(() => availableModels.value.filter(model => model.status === 'loaded'));
    const activeAnalysisSources = computed(() => analysisSources.value.filter(source => source.status === 'active'));
    const recentAnalysisResults = computed(() => analysisResults.value
        .slice()
    ...
    const stopAnalysis = async (taskId) => {
        ...
        analysisTasks.value = analysisTasks.value.map(task => task.task_id === taskId ? { ...task, status: 'stopped' } : task);
    };
    ...
    const fetchAnalysisResults = async (taskId, limit = 50) => {
        try {
            ...
            const results = await apiCall(`/analysis/results${query ? `?${query}` : ''}`);
            if (Array.isArray(results)) {
                for (const result of results) {
                    const key = `${result.source_id}-${result.request_id ?? result.timestamp}`;
                    const exists = analysisResults.value.find(existing => `${existing.source_id}-${existing.request_id ?? existing.timestamp}` === key);
                    if (!exists) {
    ...
    const removeAnalysisSource = async (sourceId) => {
        ...
        analysisSources.value = analysisSources.value.filter(source => source.source_id !== sourceId);
    };
    // system info
    const getSystemInfo = async () => apiCall('/system/info');
    const getPerformanceStats = async () => apiCall('/system/stats');
    // helpers
    const getTaskBySourceId = (sourceId) => analysisTasks.value.find(task => task.source_id === sourceId && task.status === 'running');
    const getResultsBySourceId = (sourceId, limit = 10) => (analysisResults.value
