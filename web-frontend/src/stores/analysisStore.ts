...
export const useAnalysisStore = defineStore('analysis', () => {
  ...
  const startAnalysis = async (sourceId: string, modelId: string, analysisType: string = 'object_detection') => {
    try {
      ...
      if (Array.isArray(data)) {
        data.forEach((result: AnalysisResult) => {
          const exists = analysisResults.value.find(
            r => r.request_id === result.request_id && r.timestamp === result.timestamp
          )
          if (!exists) {
  ...
  const removeAnalysisSource = async (sourceId: string) => {
    try {
      ...
      // 从列表中移除
      const index = analysisSources.value.findIndex(s => s.source_id === sourceId)
      if (index > -1) {
  ...
  const getTaskBySourceId = (sourceId: string) => {
    return analysisTasks.value.find(task => task.source_id === sourceId && task.status === 'running')
  }
